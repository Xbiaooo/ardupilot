...
const AP_Scheduler::Task Copter::scheduler_tasks[] = {
    ...
    SCHED_TASK(update_OpenMV,        400,    100,  82),
...
void Copter::update_OpenMV()
{
    ...
    if (openmv.update())
    {
        if (millis() - last_set_pos_target_time_ms >= 500)  //2Hz
        {
            gcs().send_text(MAV_SEVERITY_INFO, "%d", openmv.flag);
            gcs().send_text(MAV_SEVERITY_INFO,"OpenMV X:%d Y:%d",openmv.cx, openmv.cy);
    ...
    //gcs().send_text(MAV_SEVERITY_INFO, "%d %d", openmv.flag, a);
    //gcs().send_text(MAV_SEVERITY_INFO, "%d", a);
